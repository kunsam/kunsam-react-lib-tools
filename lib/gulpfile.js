...
gulp.task('watch-compile-with-lib', () => {
  watch(['src/**/*.*'], f => {
    if (f.event === 'unlink') {
      const fileToDelete = f.path.replace(/\.tsx?$/, '.js');
      if (fs.existsSync(fileToDelete)) {
    ...
    function onFinish() {
      ...
      if (target) {
        ...
        shell.cp('-R', [`${cwd}/lib`, `${cwd}/dist`], targetPath);
        console.log('[kunsam-tools-watch]:', 'lib-target updated')
      }
    }
    compile(undefined, srcObejct, onFinish);
  });
});
