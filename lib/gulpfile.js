...
gulp.task('compile-with-lib', gulp.series(
  done => {
    console.log('[Parallel] Compile static to js...');
    compile(undefined, undefined, '/static').on('finish', done);
  },
  done => {
    console.log('[Parallel] Compile components to js...');
    compile(undefined, undefined, '/components').on('finish', done);
  },
  done => {
    console.log('[Parallel] Compile story to js...');
    compile(undefined, undefined, '/story').on('finish', (...args) => {
      done();
    });
  }
));

gulp.task('watch-compile-with-lib', gulp.series((done) => {
  console.log('【提示，如果是增加或删除了import，可能需要两次保存才能正确更新到lib-target】')
  done();
}, () => {
  watch(['src/**/*.*'], f => {
    if (f.event === 'unlink') {
    ...
    function onFinish() {
      ...
      if (target) {
        ...
        const targetPath = path.join(cwd, `${target}/node_modules/gm-web-excel-table`);
        runCmd('cp', ['-R', distPath, targetPath], () => { });
        runCmd('cp', ['-R', libPath, targetPath], () => {
          console.timeEnd('compile total time: ')
        });
      }
    }

    compile(undefined, srcObejct, '/components',
      compile(undefined, srcObejct, '/story', onFinish)
    );
    // compile(undefined, srcObejct, 'components', onFinish);
  });
}));
